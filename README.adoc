...
#### CDP 7 - Basic encrypted 6 nodes (Kerberos / TLS)
        ...
        ./setup_machines.sh --cluster-name=${CLUSTER_NAME} \
            ...
            --paywall-password=${PAYWALL_PASSWORD} \
            --cluster-type=basic \
            --node-kts=<Dedicated Node for KTS> \
            --nodes=${NODES}  
...
#### CDP 7 - Streaming cluster encrypted (6 nodes basic with Spark 3 and Flink + a VPC of 3 nodes of Kafka/Nifi) (Kerberos / TLS)

        ./setup_machines.sh --cluster-name=${CLUSTER_NAME} \
           --license-file=${LICENSE_FILE} \
            --paywall-username=${PAYWALL_USER} \
            --paywall-password=${PAYWALL_PASSWORD} \
            --cluster-type=streaming-enc \
            --node-kts=<Dedicated Node for KTS> \
            --nodes=${NODES} 
